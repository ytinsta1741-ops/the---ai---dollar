...
- NUMBER DISCIPLINE — at most THREE numbers in the entire script, and each one must be reused, never replaced. Give Person A one headline number and one cost number, and Person B one number that beats A's despite looking smaller. Every later mention must repeat those exact figures. Do not introduce a fresh figure on a later slide, and never restate the same person's headline at a different value (a hook saying "ten grand" followed by "ninety thousand came in" for the same person makes the whole story unfollowable). The viewer is listening once, at speed, with no way to scroll back — every new number is a chance to lose them.
